...
Func _HoverButton($XBtext, $XBleft, $XBtop, $XBwidth, $XBheight, $XBcolor = "")
	If Not FileExists($XButton_Location & "\") Then
		...
		Return
	EndIf
	If not FileExists(@ScriptDir &"\Data\button_normal.png") Then
	 ...
	 $XBtop = $XBtop * $DPI
	 $XBwidth = $XBwidth * $DPI
	 $XBheight = $XBheight * $DPI

	Local $Xbtn[2]
	$Xbtn[0] = GUICtrlCreatePic(@ScriptDir & "\Data\wintop.jpg", $XBleft, $XBtop, $XBwidth, $XBheight)
	_SetImage($Xbtn[0], @ScriptDir & "\Data\button_normal.png")
...
Func _CheckHoverAndPressed($_GUI = "")
	...
	$tempID2 = @extended
	If $CtrlId2 = 1 Then
		_SetImage($tempID2 - 1, @ScriptDir &"\Data\button_press.png")
	ElseIf $CtrlId2 = 0 Then
		_SetImage($tempID2 - 1, @ScriptDir &"\Data\button_over.png")
	EndIf
	$CtrlId = _ControlHover(0, $_GUI)
	$tempID = @extended
	If $CtrlId = 1 Then
		_SetImage($tempID - 1, @ScriptDir &"\Data\button_over.png")
	ElseIf $CtrlId = 0 Then
		_SetImage($tempID - 1, @ScriptDir &"\Data\button_normal.png")
	EndIf
