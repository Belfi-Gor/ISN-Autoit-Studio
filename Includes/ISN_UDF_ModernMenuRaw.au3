...
;********************************************************************
; Modified by ISI360
; Modify-date: 09.12.2015, special settings for the ISN AutoIt Studio
;********************************************************************
#cs
...
; Set the text of an menu item
;********************************************************************
Func _GUICtrlODMenuItemSetText($nMenuID, $sText, $handle = "")
	If $nMenuID = -1 Then $nMenuID = $MENULASTITEM
	If $nMenuID <= 0 Then Return 0
    if not IsArray($arMenuItems) then return 0
    if $handle = "" then $handle = $StudioFenster_MenuHandle

	Local $i

	For $i = 1 To $arMenuItems[0][0]
		If $arMenuItems[$i][0] = $nMenuID Then
		   if $arMenuItems[$i][1] = $sText then ContinueLoop ;by isi360 (item has already the needed text)
			$arMenuItems[$i][1] = $sText
			_SetOwnerDrawn($arMenuItems[$i][3], $nMenuID, $sText, FALSE)
;~ 			GUICtrlSetData($nMenuID, $sText) ;removed...flickering
			_GUICtrlMenu_SetItemText ($handle, $nMenuID, $sText ,false) ;by isi360 (prevent flickering)
			_SetOwnerDrawn($arMenuItems[$i][3], $nMenuID, $sText)
			Return 1
		EndIf
	Next
	...
	For $i = 1 To $arMenuItems[0][0]
		If $arMenuItems[$i][3] = $hMenu Then
			...
			If $arString[0] > 1 Then
				...
				If $nAccWidth > $nMaxAccWidth OR $hMenu = "TOP" Then $nMaxAccWidth = $nAccWidth
			EndIf
			...
			If $nWidth > $nMaxWidth OR $hMenu = "TOP" Then $nMaxWidth = $nWidth
		EndIf
	Next
EndFunc
...
Func ImageList_Create($nImageWidth, $nImageHeight, $nFlags, $nInitial, $nGrow)
	Local $hImageList = DllCall($hComctl32Dll, "hwnd", "ImageList_Create", _
														...
														"int", $nInitial, _
														"int", $nGrow)
	if IsArray($hImageList) then Return $hImageList[0]
EndFunc
...
Func ImageList_AddIcon($hIml, $hIcon)
	Local $nIndex = DllCall($hComctl32Dll, "int", "ImageList_AddIcon", _
													"hwnd", $hIml, _
													"hwnd", $hIcon)
	if IsArray($nIndex) then Return $nIndex[0]
EndFunc
...
Func ImageList_Destroy($hIml)
	Local $bResult = DllCall($hComctl32Dll, "int", "ImageList_Destroy", _
													"hwnd", $hIml)
	if IsArray($bResult) then Return $bResult[0]
EndFunc
...
Func Shell_NotifyIcon($nMessage, $pNID)
	Local $nResult = DllCall($hShell32Dll, "int", "Shell_NotifyIconW", _
													"int", $nMessage, _
													"ptr", $pNID)
	if IsArray($nResult) then Return $nResult[0]
EndFunc
