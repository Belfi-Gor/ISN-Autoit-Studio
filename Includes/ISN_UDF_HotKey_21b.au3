...
Global Const $HK_WM_HOTKEY = _WinAPI_RegisterWindowMessage(_WinAPI_CreateGUID()) ;For ISN AutoIt Studio
;~ Global Const $HK_WM_HOTKEY = _WinAPI_RegisterWindowMessage('{509ADA08-BDC8-45BC-8082-1FFA4CB8D1C8}') ;original Value
...
;~ GUIRegisterMsg($HK_WM_ACTIVATE, 'HK_WM_ACTIVATE') ;by ISI360
...
Func __HK_Hook($iCode, $wParam, $lParam)
	...
	If ($iCode > -1) And ($hkTb[0][1] = 0) And ($hkTb[0][3] = 0) Then
		...
		Case $WM_KEYDOWN, $WM_SYSKEYDOWN
			;_Write_ISN_Debug_Console("Hotkey Hook reacts to key press! (Keycode: "&$vkCode&")", $ISN_Debug_Console_Errorlevel_Info, $ISN_Debug_Console_Linebreak, $ISN_Debug_Console_Insert_Time, $ISN_Debug_Console_Insert_Title, $ISN_Debug_Console_Category_Hotkey)
				If $hkTb[0][8] = 1 Then
					Return -1
				EndIf
				Switch $vkCode
...
Func HK_WM_HOTKEY($hWnd, $iMsg, $wParam, $lParam)
	...
	Switch $hWnd
		Case $hkTb[0][10]
			...
			Else
				_Write_ISN_Debug_Console("Hotkey press detected! 16-bit Key Code: "&$Key&" -> Call to func '"&$hkTb[$lParam][1]&"'", $ISN_Debug_Console_Errorlevel_Info, $ISN_Debug_Console_Linebreak, $ISN_Debug_Console_Insert_Time, $ISN_Debug_Console_Insert_Title, $ISN_Debug_Console_Category_Hotkey)
				Call($hkTb[$lParam][1])
			EndIf
			$hkTb[$lParam][8] -= 1
			$hkTb[0][11] -= 1
